...
- [ ] R1 页面容错：分离读取错误和后台分析错误，已有各视图可读；增加进度、尝试次数、暂停原因和续跑入口。
- [ ] R2 持久化：新增 D1 运行/分段/请求记录与原子预算、并发租约；导入旧缓存和尝试，不改变原文/模型/提示词/分段。
- [ ] R3 执行链：独立分析 Workflow，逐段步骤、显式退避、存储恢复及最终幂等提交；每段最多3次，整篇额外10次，费用不确定请求也计数。
- [ ] R4 接口兼容：查询、用户续跑、URL/MCP发布和修复入口统一调度；新旧执行互斥，默认关闭，指定文章灰度。
- [ ] R5 自动回归与故障注入：权限、重复提交、预算并发、超时/限流/格式、存储中断、迟到响应、最终提交失败、页面仍可读。
- [ ] R6 迭代预览：1440×960 / 390×844 真实浏览器；全量测试、类型、构建和安全复核。
- [ ] R7 Dockerfile 检查：视频 Container 不变，仅新增 Worker Workflow 与 D1 状态。
- [ ] R8 注册发布：干净发布、迁移和默认关闭的新执行链；只启用目标文章，导入19段并续跑至30/30或明确预算/永久错误暂停；校验指纹、积分和调用次数。
