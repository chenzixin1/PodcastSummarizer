...
- [ ] P1 无损兼容模型超过12条的完整双语要点：合并相邻要点，不删文字、不补译文、不放宽MCP校验。
- [ ] P2 已付费拒绝输出重新校验后才允许格式暂停续跑；失败原始输出通过 Workflow 独立存储步骤保留，历史次数不清零。
- [ ] P3 迭代预览：解析、缺字段拒绝、预算和存储恢复测试与类型检查。
- [ ] P4 Dockerfile检查：容器不变。
- [ ] P5 注册发布：部署并尝试恢复七篇；真实缺失内容或预算阻挡明确报告，不扩额。
