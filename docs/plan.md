...
- [ ] J1 扩展 `GET /api/watchless/jobs`：兼容数组字段，添加有界分页和状态过滤，强制所有者约束及 private no-store。
- [ ] J2 新增 `/watchless/jobs`：加载、登录、错误、空态、分页、阶段、失败原因、积分及详情/文章链接；首页和详情页提供入口。
- [ ] J3 迭代预览：权限、分页、筛选、过期响应测试；类型检查和真实浏览器布局验证。
- [ ] J4 Dockerfile 检查：纯页面/API 变更，不改容器或上一轮六小时时长发布状态。
- [ ] J5 注册发布：部署页面/API 时跳过容器，记录测试与实际发布状态。
