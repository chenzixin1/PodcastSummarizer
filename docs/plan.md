...
- [ ] T1 共享提取：完整内容证据、英文词边界、具体主题排序和 FDE/招聘/交付别名。
- [ ] T2 URL / MCP 发布与完整分析保存统一持久化标签及筛选关系。
- [ ] T3 历史标签只读预览、备份、带源版本保护的回填。
- [ ] T4 测试、类型检查、七项代码审查。
- [ ] T5 迭代预览：首页和分类筛选桌面/手机实测。
- [ ] T6 Dockerfile 检查：沿用现有视频容器。
- [ ] T7 注册发布：Cloudflare 上线与生产标签回读。
