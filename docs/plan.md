...
- [ ] H1 核对历史阅读强调逻辑，建立共享的非改写强调层；保留原有加粗和全文字符，限制强调密度。
- [ ] H2 接入普通与 Watchless 的中文/英文/中英对照/词汇提示；保护生词释义、自动发音与原有字体风格。
- [ ] H3 迭代预览：定向与全量回归、类型检查；桌面手机实际验证正文强调与词汇交互。
- [ ] H4 Dockerfile检查：仅呈现层，视频容器不变。
- [ ] H5 注册发布：由授权补试代理统一整合发布，生产验证后记录版本与截图。

## 四段一次性授权补试（2026-09-06）

用户确认 go：仅 140ek7gjjoe / byv311hdohe / krboguz54vw / kwhgfwostoq 当前失败段各一次。保留所有历史计数；不授权其他失败段或33/34预算暂停文章扩额。
- [ ] A1 D1一次性授权表绑定run/part/next attempt；原子保留与并发约束，默认预算不变。
- [ ] A2 调度仅在未消费授权存在时恢复，失败后不自动再补；保留原文、分段和模型。
- [ ] A3 迭代预览：测试授权消费、重复提交、过期/错误段拒绝、四次后停止及类型构建。
- [ ] A4 Dockerfile检查：容器不变。
- [ ] A5 注册发布：先兼容迁移再部署，精确登记4笔授权、逐篇执行并回读实际请求结果。
