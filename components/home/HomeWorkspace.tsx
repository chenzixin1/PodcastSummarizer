...
function SummaryCover({ item }: { item: SummaryItem }) {
  const coverSpec = useMemo(() => buildEditorialCoverSpec(item), [item]);
...
export default function HomeWorkspace({
  ...
  const explicitViewRef = useRef(hasExplicitView);
  const navigationPropsRef = useRef({ hasExplicitView, initialTag, initialView });
  ...
  const [exploreItems, setExploreItems] = useState<SummaryItem[]>(() => (
    initialExploreRows.map((row) => mapPodcastRow(row, 'explore'))
  ));
  const [myPage, setMyPage] = useState(0);
  const [explorePage, setExplorePage] = useState(1);
  const [hasMoreMy, setHasMoreMy] = useState(true);
  const [hasMoreExplore, setHasMoreExplore] = useState(initialExploreRows.length === SUMMARY_PAGE_SIZE);
  ...
  useEffect(() => {
    const previous = navigationPropsRef.current;
    if (
      previous.hasExplicitView === hasExplicitView &&
      previous.initialTag === initialTag &&
      previous.initialView === initialView
    ) {
      return;
    }

    navigationPropsRef.current = { hasExplicitView, initialTag, initialView };
    explicitViewRef.current = hasExplicitView;
    setSelectedTag(initialTag);
    setView(
      hasExplicitView || status === 'loading'
        ? initialView
        : status === 'authenticated'
          ? 'my'
          : 'explore',
    );
  }, [hasExplicitView, initialTag, initialView, status]);

  useEffect(() => {
    if (explicitViewRef.current || status === 'loading') {
      return;
    }
    setView(status === 'authenticated' ? 'my' : 'explore');
  }, [status]);
  ...
  const visibleItems = useMemo(() => {
    const filtered = filterSummaryItems(sourceItems, { view, selectedTag, query: deferredQuery });
    return sortSummaryItems(filtered, sortBy, sortDirection);
  }, [deferredQuery, selectedTag, sortBy, sortDirection, sourceItems, view]);
        ...
        <section className="min-w-0 space-y-5">
          ...
          {view === 'my' && status === 'unauthenticated' ? (
            <section className="dashboard-panel rounded-lg p-10 text-center">
              ...
              <Link href="/auth/signin?callbackUrl=/?view=my" prefetch={false} className="mt-5 inline-flex rounded-lg bg-[var(--btn-primary)] px-5 py-2.5 text-sm font-semibold text-[var(--btn-primary-text)] hover:bg-[var(--btn-primary-hover)]">
                Sign in
              </Link>
            </section>
          ) : showInitialLoading ? (
