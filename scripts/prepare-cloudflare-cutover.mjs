...
async function main() {
  ...
  const commands = [
    ...
    '',
    '# Apply all production D1 migrations before any Worker can serve infographic jobs:',
    `npx wrangler d1 migrations apply PODSUM_DB --remote --config ${productionConfigPath}`,
    '',
